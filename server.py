...
def answer(sender_id, buzz_lock, event, message):
    # ensure if the person who sent the answer token actually has the lock (do once buzz has been implemented)
    if sender_id != buzz_lock:
        return False

    # find who the host is
    host_client = clients[sender_id]
    # for client_id in clients:
    #     if clients[client_id].player_data.is_host:
    #         host_client = clients[client_id]
    #         break
...
# Returns player number who gets the lock and starts the timer thread for which people can answer within that time
def buzzing(client_id, buzz_lock, time_thread):
    # if already assigned, return same buzzer lock and don't start a timer thread
    if buzz_lock is not None:
        return buzz_lock

    time_thread.start()

    # send who buzzed to all clients
    global clients
    for id in clients:
        send_data_to_client(clients[id], "String", str(client_id))

    # return who now has the buzz lock
    return client_id
...
if __name__ == "__main__":
    ...
    # will equal a client_id/player number, None if not assigned
    buzz_lock = None
    ...
    while game_loop:
        ...
        if message == "Timeout" and buzz_lock is not None:
            buzz_lock = None
        ...
        # if message == "timeout" and gamestate == "waitingforanswer":
       ...
       #Token Parse------------------------------------------------------------------
        ...
        if (tokens[0] == "Buzzing"):
            buzz_lock = buzzing(PlayerNumber[addr][0], buzz_lock, time_thread)
            print(buzz_lock)
            ...
            answer_success = answer(PlayerNumber[addr][0], buzz_lock, event, tokens[1])
            print(answer_success)

            # remove lock and and recreate thread instance (not start it) if answer was successful
            if answer_success:
                buzz_lock = None
                time_thread = threading.Thread(target=buzz_timer, args=(message_queue,))
